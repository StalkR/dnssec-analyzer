...
func parse(p string) (Analysis, error) {
	...
	if headers == nil {
		return nil, fmt.Errorf("dnssec: headers not found")
	}
	...
	for _, header := range headers {
		...
		if checks == nil {
			return nil, fmt.Errorf("dnssec: checks not found")
		}
		var results []Result
		for _, check := range checks {
			...
			case "/red.png":
				status = ERROR
			default:
				return nil, fmt.Errorf("dnssec: unknown status color %q", check[1])
			}
